...
ADDR_AMP_SETTING   = 0x00
ADDR_MEMS_FREQ_X_L = 0x01
ADDR_MEMS_FREQ_X_H = 0x02
ADDR_MEMS_FREQ_Y_L = 0x03
ADDR_MEMS_FREQ_Y_H = 0x04
ADDR_STEP_SIZE_X   = 0x05
ADDR_STEP_SIZE_Y   = 0x06
ADDR_TDC_AREA_X    = 0x10
ADDR_TDC_AREA_Y    = 0x11
...
async def test_write_path(dut):
    ...
    await spi_write(dut, ADDR_AMP_SETTING, [0xA5])
    assert int(dut.amp_setting.value) == 0xA5, \
        f"amp_setting: expected 0xa5, got {int(dut.amp_setting.value):#04x}"
    cocotb.log.info("PASS amp_setting write")

    await spi_write(dut, ADDR_MEMS_FREQ_X_L, [0x34, 0x12])
    assert int(dut.mems_freq_x.value) == 0x1234, \
        f"mems_freq_x: expected 0x1234, got {int(dut.mems_freq_x.value):#06x}"
    cocotb.log.info("PASS 16-bit FCW burst write (auto-increment)")

    await spi_write(dut, ADDR_STEP_SIZE_X, [0x07])
    assert int(dut.step_size_x.value) == 0x07, "step_size_x write"
    cocotb.log.info("PASS step_size_x write")
...
async def test_default_values(dut):
    ...
    assert int(dut.amp_setting.value) == 0x00, "amp default"
    assert int(dut.mems_freq_x.value) == 0x0000, "freq x default"
    assert int(dut.mems_freq_y.value) == 0x0000, "freq y default"
    assert int(dut.step_size_x.value) == 0x01, "step x default"
    assert int(dut.step_size_y.value) == 0x01, "step y default"
...
async def test_unmapped_write_ignored(dut):
    ...
    await spi_write(dut, ADDR_AMP_SETTING, [0x3C])
    await spi_write(dut, 0x7F, [0xFF])          # unmapped
    assert int(dut.amp_setting.value) == 0x3C, \
...
async def test_cs_abort_recovery(dut):
    ...
    await spi_write(dut, ADDR_AMP_SETTING, [0x5A])
    assert int(dut.amp_setting.value) == 0x5A, \
        f"post-abort write failed: {int(dut.amp_setting.value):#04x}"
